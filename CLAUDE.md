...
- `public/` — static assets copied as-is at build time (video, images, documents, `api/`).
...
- `dist/` — build output. Generated by `npm run build`, uploaded to the hosting root, never committed.

## Editing rule

Edit `src/`, never `dist/` and never files on the server. A change made directly in a built
`index.html` is invisible to the build and disappears the next time it runs. This has already
happened once: the deployed site diverged from source and had to be reconstructed.

`src/styles/refinements.css` is an override layer loaded after every other sheet; its cascade
position is deliberate. `src/js/enhancements.js` holds page-level progressive enhancements that
attach after the scroll engine boots.

Secrets never enter this repository. `api/lead.php` reads its Telegram token from
`renuvol-config.php` (gitignored, lives on the server) or from the environment.
